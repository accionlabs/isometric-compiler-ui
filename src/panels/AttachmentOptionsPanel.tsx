...
const AttachmentOptionsPanel: React.FC<AttachmentOptionsPanelProps> = ({
    ...
    const [filteredAttachmentPoints, setFilteredAttachmentPoints] = useState<
        string[]
    >([]);
    ...
    useEffect(() => {
        const updateFilteredAttachmentPoints = () => {
            ...
            const newFilteredPoints = availableAttachmentPoints.filter(
                (point) => point.startsWith(prefix)
            );
            setFilteredAttachmentPoints(newFilteredPoints);

            if (
                selectedAttachmentPoint &&
                (newFilteredPoints.length === 0 ||
                    !newFilteredPoints.includes(selectedAttachmentPoint))
            ) {
    ...
    const attachmentPointOptions = useMemo(
        () => [
            ...
            ...filteredAttachmentPoints.map((point) => ({
                value: point,
                label: point
            }))
        ],
