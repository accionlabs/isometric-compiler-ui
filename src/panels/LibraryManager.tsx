...
const LibraryManager: React.FC<LibraryManagerProps> = ({
    ...
    const { data: libraries = [] } = useQuery({
        queryKey: ["libraries", typeOfLibrary],
        queryFn: () => getLibraries(typeOfLibrary)
    });
    const { mutate: createShapesMutation } = useMutation({
        ...
        onSuccess: (newLibrary) => {
            queryClient.setQueryData<LibraryData[]>(
                ["libraries"],
                (oldLibraries = []) => {
                    const index = oldLibraries.findIndex(
                        (lib) => lib.id === newLibrary.id
                    );
                    if (index === -1) {
    ...
    const handleSubmitLibrary = useCallback(async () => {
        try {
            ...
            createShapesMutation({
                ...(editingLibraryId && { id: editingLibraryId }),
                name: libraryForm.name,
                description: libraryForm.description,
                updateMode: libraryForm.updateMode,
