...
const LibraryManager: React.FC<LibraryManagerProps> = ({
  ...
  const handleActivateLibrary = useCallback(async (libraryId: string) => {
    ...
    try {
      ...
      if (shapes.length === 0 && library.source) {
        if (library.source.type === 'googledrive' && 
          ...
          setLoadingStatus('Please use Edit function to load local shapes');
          setTimeout(() => {
  ...
  const handleReloadLibrary = useCallback(async (libraryId: string) => {
    ...
    try {
      ...
      if (!library || !library.source) {
        throw new Error('Library not found or has no source configuration');
      }
      ...
      setLoadingStatus(`Error reloading library: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      setTimeout(() => {
        setIsLoadingDialogOpen(false);
        setLoadingStatus(null);
  ...
  const processLocalFiles = async (
    ...
    const existingShapeMap = new Map(existingShapes.map(shape => [shape.name, shape]));
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (svgFileObj) {
        ...
        const svgContent = await SVGLibraryManager.readFileAsText(svgFileObj);
  ...
  const handleUpdateLibrary = useCallback(async (
    libraryId: string,
    shapes: Shape[],
    source: { type: 'googledrive' | 'local'; googleDrive?: any; local?: any }
  ) => {
    ...
    try {
      ...
      if (libraryForm.source === 'googledrive') {
        ...
        if (libraryForm.indexFile && libraryForm.svgFiles.length > 0) {
          const indexContent = await SVGLibraryManager.readFileAsText(libraryForm.indexFile);
      ...
      setLoadingStatus(`Error processing library: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      setTimeout(() => {
        setIsLoadingDialogOpen(false);
        setLoadingStatus(null);
        ...
        <DialogContent className="bg-gray-800 border border-gray-700">
          <DialogHeader>
            <DialogTitle className="text-white">
              {editingLibraryId ? 'Edit Library' : 'Create New Library'}
            </DialogTitle>
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input
                ...
                onChange={(e) => setLibraryForm(prev => ({
                  ...prev,
                  name: e.target.value
                }))}
            ...
            {editingLibraryId && (
              <div>
                <label className="block text-sm font-medium text-gray-200 mb-2">Update Mode</label>
                <ToggleGroup
                  options={updateModeOptions}
                  value={libraryForm.updateMode}
                  onValueChange={(value) => setLibraryForm(prev => ({
                    ...prev,
                    updateMode: value as 'replace' | 'append'
                  }))}
                />
              </div>
            )}
              ...
              <>
                <div>
                  ...
                  <Input
                    ...
                    className="w-full bg-gray-700 text-white border-gray-600"
                  />
                  {editingLibraryId && !libraryForm.indexFile && (
                    <p className="text-sm text-gray-400 mt-1">
                      Leave empty to keep existing index
                    </p>
                  )}
                </div>
                <div>
                  ...
                  <Input
                    ...
                    className="w-full bg-gray-700 text-white border-gray-600"
                  />
                  {editingLibraryId && libraryForm.svgFiles.length === 0 && (
                    <p className="text-sm text-gray-400 mt-1">
                      {libraryForm.updateMode === 'append'
            ...
            <div className="flex justify-end space-x-2">
              ...
              <Button
                ...
                disabled={!libraryForm.name.trim() || (
                  libraryForm.source === 'googledrive' ?
                    !libraryForm.spreadsheetUrl || !libraryForm.folderUrl :
                    (!editingLibraryId && (!libraryForm.indexFile || libraryForm.svgFiles.length === 0))
                )}
                className="bg-blue-600 hover:bg-blue-700 text-white disabled:bg-blue-800 disabled:opacity-50"
              >
                {editingLibraryId ? 'Update Library' : 'Create Library'}
              </Button>
            </div>
          </div>
        </DialogContent>
