...
export function findConcaveHull<P extends Point>(points: P[]): P[] {
    ...
    let start = uniquePoints.reduce((min, p) =>
    ...
    do {
        ...
        for (let i = 1; i < uniquePoints.length; i++) {
            if (uniquePoints[i] === currentPoint) continue;

            const cross =
                (uniquePoints[i].x - currentPoint.x) *
                    (nextPoint.y - currentPoint.y) -
                (uniquePoints[i].y - currentPoint.y) *
            ...
            if (
                ...
                cross > 0 ||
                (cross === 0 &&
                    distance(currentPoint, uniquePoints[i]) <
        ...
        // Break if we exceed maximum iterations
        if (iteration > maxIterations) {
            console.warn("Maximum iterations reached in findConcaveHull");
            break;
        }
    } while (currentPoint !== start);

    const uniqueHullPoints = removeDuplicatePoints(hull);

    return uniqueHullPoints;
}

export function addPointsToHullSegments<P extends Point>(
    hull: P[],
    allPoints: P[],
    maxDistance: number
): P[] {
    ...
    const getPerpendicularDistance = (point: P, start: P, end: P): number => {
    ...
    // Helper to check if a point is already in result array
    const isPointInResult = (point: P): boolean => {
        return result.some(p => p.x === point.x && p.y === point.y);
    };

    // Helper to sort points along a segment
    const sortPointsAlongSegment = (points: P[], start: P, end: P): P[] => {
        const vectorX = end.x - start.x;
        const vectorY = end.y - start.y;
        
        return points.sort((a, b) => {
            // Project points onto the segment vector
            const projA = (a.x - start.x) * vectorX + (a.y - start.y) * vectorY;
            const projB = (b.x - start.x) * vectorX + (b.y - start.y) * vectorY;
            return projA - projB;
        });
    };

    // Process each segment of the hull
    for (let i = 0; i < hull.length; i++) {
        const start = hull[i];
        const end = hull[(i + 1) % hull.length];
        
        // Add start point
        if (!isPointInResult(start)) {
            result.push(start);
        }

        // Find points close to this segment
        const nearPoints = allPoints.filter(point => {
            // Skip points that are already hull vertices
            if (hull.some(p => p.x === point.x && p.y === point.y)) {
                return false;
            }

            // Check if point is within maxDistance of the segment
            const distance = getPerpendicularDistance(point, start, end);
            return distance <= maxDistance;
        });

        // Sort points along the segment and add them
        if (nearPoints.length > 0) {
            const sortedPoints = sortPointsAlongSegment(nearPoints, start, end);
            for (const point of sortedPoints) {
                if (!isPointInResult(point)) {
                    result.push(point);
                }
            }
        }
...
function getHullSegmentOld<P extends Point>(
    hull: P[],
    direction1: Direction,
    direction2: Direction
): P[] {
...
export function createGridPoints(
    ...
    while (remainingPoints.length > 0) {
        ...
        const outerHull = findConcaveHull(remainingPoints);
        // Add points to the hull which lie on the segments;
        const hull = addPointsToHullSegments(outerHull, remainingPoints, 5);
    ...
    console.log("grid debug:", debugPoints);
    return gridPoints;
}
